...
#    Modified time::   2026.05.27
#    Modification ::
#                (2) set the density color scale from the current depth slice.
#####
...
read lscalev hscalev cpt_step tick_step <<EOF
$(awk '
  NR == 1 { min = $3; max = $3 }
  $3 < min { min = $3 }
  $3 > max { max = $3 }
...
' "${velfile}")
EOF
echo "----density color bar scale $lscalev $hscalev tick $tick_step ---- "
velscale="$lscalev/$hscalev/$cpt_step"
...
#gmt psscale  -B${tick_step}/:g/cm3:  -Dx5.6i/2.3i/4.6i/0.2i -C${cptfile}  -O  -K  -S -V >> ${FNAME}
gmt psscale  -B${tick_step}/:g/cm3:  -Dx5.6i/2.6i/4.6i/0.2i -C${cptfile}  -O  -K  -S -V >> ${FNAME}
